...
class HouseCat1:
    ...
    @classmethod
    def from_pounds(cls, name, weight):
        return cls(name, age, weight / 2.2046)
...
# With the Animal base class, House cat Derivative class doesn't need any explicit methods defined.
class HouseCat(Animal):
    ''' Models a standard house cat. '''
    says = 'meow'

    # to overide the base class's __init__ method
    def __init__(self, name, age, weight_kg, extrovertion_scale=3):
        print(f'Animal.__init__({name=}, {age=}, {weight_kg=}, {extrovertion_scale=})')
        super().__init__(name, age, weight_kg)
        # name, age, weight can be set from the super callable
        # self.name = name
        # self.age = age
        # self.weight_kg = weight_kg
        # set the level of introversion / extroversion
        # 0 = Introvert / 5 = Extrovert
...
if __name__ == '__main__':
    ...
    print(kara.play(ada, gus, pogo, rolo))
    print(pogo.play(kara, gus, pogo, rolo))
    ...
    print(f' {jojo.name} is a Animal: {isinstance(jojo, Animal)}')
