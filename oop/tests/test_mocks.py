...
def resource_finder(ref: str, source: str) -> dict:
    return {
        'ref': ref,
        'source': source,
        'price': random.random() * random.randint(1, 10)
        }

class PriceLocator:
    def price_of(self, ref: str, source: str, finder: callable = resource_finder) -> float:
        ''' Simulate returning a price for a fake resources. '''
...
class PriceLocatorTestCase(unittest.TestCase):
    def test_price_of(self):
        ...
        price: PriceLocator = PriceLocator()
        ...
        # In this test knowing that the finder callabel is provided with the correct arguments
        # is more important that connecting to the "external service" simulated with resource_finder.
